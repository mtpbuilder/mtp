...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for mtpio reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("mtpio"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("mtpio"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with mtpio reqauth
   chain.unlink_authority(name("alice"), name("mtpio"), name("reqauth"));
   ...
   // Link authority for any mtpio action with alice's scud key
   chain.link_authority(name("alice"), name("mtpio"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("mtpio"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with mtpio. prefix with privileged account
   chain.create_account(name("mtpio.test1"));

   // Creating account with mtpio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("mtpio.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'mtpio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("mtpio"), name("mtpio.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("mtpio"), name("mtpio.any"), name("reqauth") );

   /// this should succeed because mtpio::reqauth is linked to any permission
   chain.push_reqauth(name("alice"), { permission_level{N(alice), name("spending")} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = mtpio::chain::resource_limits::resource_usage_object;
   using by_owner = mtpio::chain::resource_limits::by_owner;
