...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(mtpio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(mtpio.token), contracts::mtpio_token_wasm() );
   c.set_abi( N(mtpio.token), contracts::mtpio_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( N(mtpio.token), N(create), N(mtpio.token), mutable_variant_object()
              ("issuer",       "mtpio" )
              ("maximum_supply", core_from_string("10000000.0000"))
      );

   cr = c.push_action( N(mtpio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "mtpio" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
      );

   cr = c.push_action( N(mtpio.token), N(transfer), config::system_account_name, mutable_variant_object()
              ("from",     "mtpio")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
