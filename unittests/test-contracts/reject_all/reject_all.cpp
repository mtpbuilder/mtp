...
extern "C" {
   void apply( uint64_t receiver, uint64_t first_receiver, uint64_t action ) {
      ...
      //   * do not reject an mtpio::setcode that sets code on the mtpio account unless the rejectall account exists;
      //   * do not reject an mtpio::newaccount that creates the rejectall account.

      if( first_receiver == "mtpio"_n.value ) {
         if( action == "setcode"_n.value ) {
            auto accnt = unpack_action_data<name>();
            if( accnt == "mtpio"_n && !is_account("rejectall"_n) )
               return;
