#include "proxy.hpp"
#include <mtpio/transaction.hpp>

using namespace mtpio;

proxy::proxy( mtpio::name self, mtpio::name first_receiver, mtpio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      mtpio::token::transfer_action a( "mtpio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, mtpio::ignore<std::vector<char>> ) {
