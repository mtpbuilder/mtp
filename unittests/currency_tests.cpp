...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(mtpio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(mtpio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::mtpio_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time ))
      {
         create_account( N(mtpio.token));
         set_code( N(mtpio.token), contracts::mtpio_token_wasm() );

         auto result = push_action(N(mtpio.token), N(create), mutable_variant_object()
                 ("issuer",       mtpio_token)
         ...
         result = push_action(N(mtpio.token), N(issue), mutable_variant_object()
                 ("to",       mtpio_token)
      ...
      static const name mtpio_token;
};

const name currency_tester::mtpio_token = N(mtpio.token);
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(mtpio.token), expected.get_symbol(), N(mtpio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(mtpio.token), N(transfer), mutable_variant_object()
         ("from", mtpio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(mtpio.token), N(transfer), mutable_variant_object()
                                    ("from", mtpio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action(N(alice), N(transfer), data),
                             mtpio_assert_message_exception, mtpio_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      BOOST_CHECK_EXCEPTION(symbol malformed(SY(6,mtp)),
                            symbol_type_exception, fc_exception_message_is("invalid symbol: mtp"));
   }

   // invalid - contains lower case characters, exception thrown
   {
      BOOST_CHECK_EXCEPTION(symbol(5,"mtp"),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(mtpio_token, N(alice), "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer(N(alice), N(carl), "20.50 USD"), mtpio_assert_message_exception);
   }
