...
struct dummy_action {
   static mtpio::chain::name get_name() {
      return N(dummyaction);
   }
   static mtpio::chain::name get_account() {
      return N(testapi);
   }
...
bool is_access_violation(fc::unhandled_exception const & e) {
   try {
    ...
    catch (const mtpio::chain::wasm_execution_error& e) {
       return true;
    } catch (...) {
...
bool is_wasm_execution_error(mtpio::chain::wasm_execution_error const& e) {return true;}
...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( config::system_account_name, contracts::mtpio_bios_wasm() );

   set_code( N(test), contracts::mtpio_bios_wasm() );
   set_abi( N(test), contracts::mtpio_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
BOOST_FIXTURE_TEST_CASE(action_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "assert_false", {} ),
                          mtpio_assert_message_exception, mtpio_assert_message_is("test_action::assert_false") );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_0", raw_bytes), mtpio::chain::wasm_execution_error,
         [](const mtpio::chain::wasm_execution_error& e) {
   ...
   // test read_action_to_64k
   raw_bytes.resize(3);
	BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_64k", raw_bytes ), mtpio::chain::wasm_execution_error,
         [](const mtpio::chain::wasm_execution_error& e) {
            return expect_assert_message(e, "access violation");
         }
      );
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_current_time", fc::raw::pack(now) ),
                          mtpio_assert_message_exception, mtpio_assert_message_is("tmp == current_time()")     );
...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, TESTER) { try {
      ...
      BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_transaction", "send_cf_action_fail", {} ),
                             ...
                             mtpio_assert_message_is("context free actions cannot have authorizations") );
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_inline_fail", {}),
                          mtpio_assert_message_exception,
                          mtpio_assert_message_is("test_action::assert_false")                          );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_recurse", {}), mtpio::chain::transaction_exception,
         [](const mtpio::chain::transaction_exception& e) {
...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) { try {
...

   {
      ...
      auth.accounts.push_back( permission_level_weight{{N(testapi), config::mtpio_code_name}, 1} );
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@mtpio.code.
      dtt_action dtt_act3;
...
BOOST_FIXTURE_TEST_CASE(multi_index_tests, TESTER) { try {
   ...
   auto check_failure = [this]( action_name a, const char* expected_error_msg ) {
      BOOST_CHECK_EXCEPTION(  push_action( N(testapi), a, N(testapi), {} ),
                              mtpio_assert_message_exception,
                              mtpio_assert_message_is( expected_error_msg )
      );
   };
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = N(active),
         .pubkeys    = {
            public_key_type(string("MTP7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
...
/*************************************************************************************
 * mtpio_assert_code_tests test cases
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(mtpio_assert_code_tests, TESTER) { try {
   produce_block();
   create_account( N(testapi) );
   produce_block();
   set_code(N(testapi), contracts::test_api_wasm() );
   ...
   const char* abi_string = R"=====(
{
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_assert_code", fc::raw::pack((uint64_t)42) ),
                          mtpio_assert_code_exception, mtpio_assert_code_is(42)                                        );
