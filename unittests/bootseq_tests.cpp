...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(mtpio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create mtpio.msig and mtpio.token
        create_accounts({N(mtpio.msig), N(mtpio.token), N(mtpio.ram), N(mtpio.ramfee), N(mtpio.stake), N(mtpio.vpay), N(mtpio.bpay), N(mtpio.saving) });
        // Set code for the following accounts:
        //  - mtpio (code: mtpio.bios) (already set by tester constructor)
        //  - mtpio.msig (code: mtpio.msig)
        //  - mtpio.token (code: mtpio.token)
        // set_code_abi(N(mtpio.msig), contracts::mtpio_msig_wasm(), contracts::mtpio_msig_abi().data());//, &mtpio_active_pk);
        // set_code_abi(N(mtpio.token), contracts::mtpio_token_wasm(), contracts::mtpio_token_abi().data()); //, &mtpio_active_pk);

        set_code_abi(N(mtpio.msig),
                     contracts::mtpio_msig_wasm(),
                     contracts::mtpio_msig_abi().data());//, &mtpio_active_pk);
        set_code_abi(N(mtpio.token),
                     contracts::mtpio_token_wasm(),
                     contracts::mtpio_token_abi().data()); //, &mtpio_active_pk);

        // Set privileged for mtpio.msig and mtpio.token
        set_privileged(N(mtpio.msig));
        set_privileged(N(mtpio.token));

        // Verify mtpio.msig and mtpio.token is privileged
        const auto& mtpio_msig_acc = get<account_metadata_object, by_name>(N(mtpio.msig));
        BOOST_TEST(mtpio_msig_acc.is_privileged() == true);
        const auto& mtpio_token_acc = get<account_metadata_object, by_name>(N(mtpio.token));
        BOOST_TEST(mtpio_token_acc.is_privileged() == true);


        // Create SYS tokens in mtpio.token, set its manager as mtpio
        ...
        create_currency(N(mtpio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to mtpio.system
        issue(N(mtpio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(mtpio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("mtpio"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), mtpio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), mtpio_assert_message_exception);
