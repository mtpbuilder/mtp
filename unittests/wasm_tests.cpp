...
BOOST_FIXTURE_TEST_CASE( basic_test, TESTER ) try {
   ...
   transaction_id_type yes_assert_id;
   {
      ...
      BOOST_CHECK_THROW(push_transaction( trx ), mtpio_assert_message_exception);
   }
...
BOOST_FIXTURE_TEST_CASE( f32_f64_overflow_tests, tester ) try {
   ...
   auto check = [&](const char *wast_template, const char *op, const char *param) -> bool {
      ...
      try {
         ...
         const auto& receipt = get_transaction_receipt(trx.id());
         return true;
      } catch (mtpio::chain::wasm_execution_error &) {
         return false;
      }
   };
   //
...
BOOST_FIXTURE_TEST_CASE( big_memory, TESTER ) try {
   ...
   string biggest_memory_wast_f = fc::format_string(biggest_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES", mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024)));
   ...
   string too_big_memory_wast_f = fc::format_string(too_big_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES_PLUS_ONE", mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024)+1));
   BOOST_CHECK_THROW(set_code(N(bigmem), too_big_memory_wast_f.c_str()), mtpio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( table_init_tests, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(tableinit), too_big_table), mtpio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( table_init_oob, TESTER ) try {
   ...
   trx.actions[0].authorization = vector<permission_level>{{N(tableinitoob),config::active_name}};
    ...
    auto pushit_and_expect_fail = [&]() {
      ...
      BOOST_CHECK_THROW(push_transaction(trx), mtpio::chain::wasm_execution_error);
   };
   ...
   BOOST_CHECK_THROW(set_code(N(tableinitoob), table_init_oob_no_table_wast), mtpio::chain::wasm_exception);
...
BOOST_FIXTURE_TEST_CASE( memory_init_border, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_toolong), mtpio::chain::wasm_execution_error);
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_negative), mtpio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( nested_limit_test, TESTER ) try {
   ...
   }
   {
      ...
      for(unsigned int i = 0; i < 1024; ++i)
         ss << ")";
      ss << "))";
      BOOST_CHECK_THROW(set_code(N(nested), ss.str().c_str()), mtpio::chain::wasm_execution_error);
   }
...
BOOST_FIXTURE_TEST_CASE( lotso_globals, TESTER ) try {
   ...
   .c_str()), mtpio::chain::wasm_execution_error);
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( offset_check, TESTER ) try {
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << mtpio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0)))";
   ...
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << mtpio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << mtpio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0)))";
      ss << ") (export \"apply\" (func $apply)) )";

      BOOST_CHECK_THROW(set_code(N(offsets), ss.str().c_str()), mtpio::chain::wasm_execution_error);
      produce_block();
   }
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << mtpio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
...
// abi_serializer::to_variant failed because mtpio_system_abi modified via set_abi.
// This test also verifies that chain_initializer::mtp_contract_abi() does not conflict
// with mtpio_system_abi as they are not allowed to contain duplicates.
BOOST_FIXTURE_TEST_CASE(mtpio_abi, TESTER) try {
   produce_blocks(2);
   ...
   // verify to_variant works on mtp native contract type: newaccount
...
BOOST_FIXTURE_TEST_CASE( check_table_maximum, TESTER ) try {
   ...
   signed_transaction trx;
   action act;
   act.name = name(mtpio::chain::wasm_constraints::maximum_table_elements+54334);
   act.account = N(tbl);
   act.authorization = vector<permission_level>{{N(tbl),config::active_name}};
   trx.actions.push_back(act);
      set_transaction_headers(trx);
   ...
   BOOST_CHECK_EXCEPTION(push_transaction(trx), mtpio::chain::wasm_execution_error, [](const mtpio::chain::wasm_execution_error &e) {return true;});
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( big_maligned_host_ptr, TESTER ) try {
   ...
   string large_maligned_host_ptr_wast_f = fc::format_string(large_maligned_host_ptr, fc::mutable_variant_object()
                                              ("MAX_WASM_PAGES", mtpio::chain::wasm_constraints::maximum_linear_memory/(64*1024))
                                              ("MAX_NAME_ARRAY", (mtpio::chain::wasm_constraints::maximum_linear_memory-1)/sizeof(chain::account_name)));
...
BOOST_FIXTURE_TEST_CASE( depth_tests, TESTER ) try {
   ...
   string wasm_depth_okay = fc::format_string(depth_assert_wasm, fc::mutable_variant_object()
                                              ("MAX_DEPTH", mtpio::chain::wasm_constraints::maximum_call_depth));
   ...
   string wasm_depth_one_over = fc::format_string(depth_assert_wasm, fc::mutable_variant_object()
                                              ("MAX_DEPTH", mtpio::chain::wasm_constraints::maximum_call_depth+1));
...
// TODO: Update to use mtp-vm once merged
...
BOOST_FIXTURE_TEST_CASE(net_usage_tests, tester ) try {
   ...
   (import "env" "mtpio_assert" (func $mtpio_assert (param i32 i32)))
      (table 0 anyfunc)
      (memory $0 1)
      ...
      produce_blocks(1);
      signed_transaction trx;
      auto wasm = ::mtpio::chain::wast_to_wasm(code);
