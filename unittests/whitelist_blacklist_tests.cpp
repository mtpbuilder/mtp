...
#include <mtpio/testing/tester_network.hpp>
...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(mtpio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(mtpio.token), contracts::mtpio_token_wasm() );
         chain->set_abi(N(mtpio.token), contracts::mtpio_token_abi().data() );
         chain->push_action( N(mtpio.token), N(create), N(mtpio.token), mvo()
              ( "issuer", "mtpio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(mtpio.token), N(issue), N(mtpio.token), mvo()
              ( "to", "mtpio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(mtpio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(mtpio.token), N(alice)};
   test.init();

   test.transfer( N(mtpio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(mtpio.token), N(bob)};
   test.init();

   test.transfer( N(mtpio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(mtpio.token) );
   ...
   test.chain->set_code(N(bob), contracts::mtpio_token_wasm() );
   test.chain->set_abi(N(bob), contracts::mtpio_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::mtpio_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::mtpio_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(mtpio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_mtpio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, contracts::mtpio_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'mtpio::onerror' is on the action blacklist")
                        );
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(mtpio::testing::base_tester::get_public_key(name("alice"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::mtpio_code_name}, 1} );

   tester1.chain->push_action( N(mtpio), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(mtpio::testing::base_tester::get_public_key(name("bob"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::mtpio_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{N(bob), config::mtpio_code_name}, 1} );

   tester1.chain->push_action( N(mtpio), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(mtpio::testing::base_tester::get_public_key(name("charlie"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(charlie), config::mtpio_code_name}, 1} );

   tester1.chain->push_action( N(mtpio), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(mtpio) && act.name == N(onblock) ) return;
