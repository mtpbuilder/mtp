...
namespace mtpio_system {

class mtpio_system_tester : public TESTER {
public:

   mtpio_system_tester()
   : mtpio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   mtpio_system_tester(Lambda setup) {
      setup(*this);
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(mtpio.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( N(mtpio.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("mtpio") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("mtpio")) + get_balance(name("mtpio.ramfee")) + get_balance(name("mtpio.stake")) + get_balance(name("mtpio.ram")) );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& mtpin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",mtpin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(mtpio.token), act, N(accounts), name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(mtpio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(mtpio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = mtpio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(mtpio.token), name(symbol_code), N(stat), name(symbol_code) );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "mtpio_global_state", data, abi_serializer::create_yield_function( abi_serializer_max_time ) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(mtpio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("mtpio"), name("mtpio.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(mtpio.msig), contracts::mtpio_msig_wasm() );
         set_abi( N(mtpio.msig), contracts::mtpio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(mtpio.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total MTP supply to activate chain
      transfer( name("mtpio"), name("alice1111111"), core_from_string("650000000.0000"), name("mtpio") );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::mtpio_code_name}, 1},
...
inline uint64_t M( const string& mtp_str ) {
   return core_from_string( mtp_str ).get_amount();
}
