...
###############################################################
# nodmtp_producer_watermark_test
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("mtpio", "setprods", setProdsStr, opts)
...
try:
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "mtpio":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
