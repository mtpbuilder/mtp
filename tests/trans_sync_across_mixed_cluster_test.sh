...
killAll()
{
  programs/mtpio-launcher/mtpio-launcher -k 15
}

cleanup()
{
    rm -rf etc/mtpio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clmtp/clmtp get info)"
  verifyErrorCode "clmtp get info"
...
# stand up nodmtp cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodmtp \"--plugin mtpio::wallet_api_plugin\" $launcherOpts
programs/mtpio-launcher/mtpio-launcher --nodmtp "--plugin mtpio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clmtp/clmtp --port $port2 get block 1)"
    verifyErrorCode "clmtp get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clmtp/clmtp create key)"
verifyErrorCode "clmtp create key"
...
PASSWORD_INITA="$(programs/clmtp/clmtp wallet create --name inita)"
verifyErrorCode "clmtp wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clmtp/clmtp wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clmtp wallet import"
programs/clmtp/clmtp wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clmtp wallet import"
programs/clmtp/clmtp wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clmtp wallet import"
programs/clmtp/clmtp wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clmtp wallet import"
...
ACCOUNT_INFO="$(programs/clmtp/clmtp create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clmtp create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clmtp/clmtp get account testera)"
verifyErrorCode "clmtp get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clmtp/clmtp transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clmtp transfer"
   ...
   TRANS_INFO="$(programs/clmtp/clmtp --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clmtp get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
