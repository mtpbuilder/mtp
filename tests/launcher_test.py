...
# launcher-test
#
# Specifically tests using the bios bootstrap script that is created by mtpio-launcher
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        ...
        killmtpInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up mtp walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.mtpioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.mtpioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.mtpioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.mtpioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.mtpioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.mtpioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.mtpioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountmtpBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountmtpBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killmtpInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
