...
class Utils:
    ...
    mtpClientPath="programs/clmtp/clmtp"
    MiscmtpClientArgs="--no-auto-kmtpd"

    mtpWalletName="kmtpd"
    mtpWalletPath="programs/kmtpd/"+ mtpWalletName

    mtpServerName="nodmtp"
    mtpServerPath="programs/nodmtp/"+ mtpServerName

    mtpLauncherPath="programs/mtpio-launcher/mtpio-launcher"
    MongoPath="mongo"
    ShuttingDown=False
    CheckOutputDeque=deque(maxlen=10)

    mtpBlockLogPath="programs/mtpio-blocklog/mtpio-blocklog"
    ...
    def getBlockLog(blockLogLocation, blockLogAction=BlockLogAction.return_blocks, outputFile=None, first=None, last=None, throwException=False, silentErrors=False, exitOnError=False):
        ...
        cmd="%s --blocks-dir %s --as-json-array %s%s%s%s" % (Utils.mtpBlockLogPath, blockLogLocation, outputFileStr, firstStr, lastStr, blockLogActionStr)
