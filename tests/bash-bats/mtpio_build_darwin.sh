...
export SCRIPT_LOCATION="scripts/mtpio_build.sh"
export TEST_LABEL="[mtpio_build_darwin]"
...
## Needed to load mtpio_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting MTPIO Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap mtpio/mtpio") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting MTPIO Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "MTPIO has been successfully built") ]] || exit
}