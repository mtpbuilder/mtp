...
SCRIPT_LOCATION="scripts/mtpio_uninstall.sh"
TEST_LABEL="[mtpio_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - MTPIO Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled MTPIO Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i mtpiotest
  [[ $output =~ .*/mtpiotest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/mtpio" ]] && [[ ! $output =~ ".local/share/mtpio" ]]) || exit
  [[ ! $output =~ "MTPIO Removal Complete" ]] || exit
}
