...
@test "${TEST_LABEL} > previous install prompt" {
  ...
  [[ -z $(echo "${output}") ]] || exit
  # Exists, prompt
  mkdir -p $MTPIO_INSTALL_DIR
  run previous-install-prompt
  [[ ! -z $(echo "${output}" | grep "MTPIO has already been installed into ${MTPIO_INSTALL_DIR}") ]] || exit
  rm -rf $MTPIO_INSTALL_DIR
}
