...
###############################################################
# nodmtp_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of nodmtp can work with newer versions of nodmtp.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, nodmtpPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, nodmtpPath=nodmtpPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraNodmtpArgs=" --plugin mtpio::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of nodmtp that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodmtpPath="programs/nodmtp/nodmtp")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodmtpPath="programs/nodmtp/nodmtp")
