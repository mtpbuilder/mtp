...
# nodmtp_run_test
#
# General test that tests a wide range of general use actions around nodmtp and kmtpd
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.mtpioAccount)
    ...
    contractDir="unittests/contracts/mtpio.token"
    wasmFile="mtpio.token.wasm"
    abiFile="mtpio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getmtpAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get MTP account failed", raw=True)
    ...
    account=node.getmtpAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getmtpAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
