#!/bin/bash
# The purpose of this test is to ensure that the output of the "nodmtp --version" command matches the version string defined by our CMake files
# If the environment variable BUILDKITE_TAG is empty or unset, this test will echo success
echo '##### Nodmtp Version Label Test #####'
...
# orient ourselves
[[ "$MTPIO_ROOT" == '' ]] && MTPIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/mtp/')
[[ "$MTPIO_ROOT" == '' ]] && MTPIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/MTPIO/mtpio/')
[[ "$MTPIO_ROOT" == '' ]] && MTPIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/build/' | sed 's,/build/,,')
echo "Using MTPIO_ROOT=\"$MTPIO_ROOT\"."
# determine expected value
CMAKE_CACHE="$MTPIO_ROOT/build/CMakeCache.txt"
CMAKE_LISTS="$MTPIO_ROOT/CMakeLists.txt"
if [[ -f "$CMAKE_CACHE" && $(cat "$CMAKE_CACHE" | grep -c 'DOXY_MTP_VERSION') > 0 ]]; then
    echo "Parsing \"$CMAKE_CACHE\"..."
    EXPECTED="v$(cat "$CMAKE_CACHE" | grep 'DOXY_MTP_VERSION' | cut -d '=' -f 2)"
...
if [[ "$EXPECTED" == '' ]]; then
    ...
    echo ''
    echo '$ cat "$CMAKE_CACHE" | grep "DOXY_MTP_VERSION"'
    cat "$CMAKE_CACHE" | grep "DOXY_MTP_VERSION"
    echo '$ pwd'
    pwd
    echo '$ ls -la "$MTPIO_ROOT"'
    ls -la "$MTPIO_ROOT"
    echo '$ ls -la "$MTPIO_ROOT/build"'
    ls -la "$MTPIO_ROOT/build"
    exit 1
fi
echo "Expecting \"$EXPECTED\"..."
# get nodmtp version
ACTUAL=$($MTPIO_ROOT/build/bin/nodmtp --version) || : # nodmtp currently returns -1 for --version
# test
