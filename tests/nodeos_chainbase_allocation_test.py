...
###############################################################
# nodmtp_chainbase_allocation_test
#
...
try:
    ...
    # (automatically taken care by the automatically generated mtpio account)
    assert cluster.launch(
        ...
        specificExtraNodmtpArgs={
            1:"--read-mode irreversible --plugin mtpio::producer_api_plugin"})
    ...
    cmd = "create account -j mtpio sample MTP6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\
         MTP6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV --delay-sec 600 -p mtpio"
    trans = producerNode.processClmtpCmd(cmd, cmd, silentErrors=False)
    assert trans

    # Schedule a new producer to trigger new producer schedule for "global_property_object"
    newProducerAcc = Account("newprod")
    newProducerAcc.ownerPublicKey = "MTP6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    newProducerAcc.activePublicKey = "MTP6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    producerNode.createAccount(newProducerAcc, cluster.mtpioAccount)

    setProdsStr = '{"schedule": ['
    setProdsStr += '{"producer_name":' + newProducerAcc.name + ',"block_signing_key":' + newProducerAcc.activePublicKey + '}'
    setProdsStr += ']}'
    cmd="push action -j mtpio setprods '{}' -p mtpio".format(setProdsStr)
    trans = producerNode.processClmtpCmd(cmd, cmd, silentErrors=False)
    assert trans
