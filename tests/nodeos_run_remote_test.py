...
###############################################################
# nodmtp_run_remote_test
#
#  Tests remote capability of the nodmtp_run_test. Test will setup cluster and pass nodes info to nodmtp_run_test. E.g.
#  nodmtp_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodmtp_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodmtp", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killmtpInstances, False, False, killAll, dumpErrorDetails)
