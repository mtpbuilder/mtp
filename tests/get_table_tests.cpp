...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(mtpio.token) )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   create_accounts({ N(mtpio.token), N(mtpio.ram), N(mtpio.ramfee), N(mtpio.stake),
      N(mtpio.bpay), N(mtpio.vpay), N(mtpio.saving), N(mtpio.names) });

   std::vector<account_name> accs{N(inita), N(initb), N(initc), N(initd)};
   create_accounts(accs);
   produce_block();

   set_code( N(mtpio.token), contracts::mtpio_token_wasm() );
   set_abi( N(mtpio.token), contracts::mtpio_token_abi().data() );
   produce_blocks(1);

   // create currency
   auto act = mutable_variant_object()
         ("issuer",       "mtpio")
         ("maximum_supply", mtpio::chain::asset::from_string("1000000000.0000 SYS"));
   push_action(N(mtpio.token), N(create), N(mtpio.token), act );

   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, mtpio::chain::asset::from_string("999.0000 SYS") );
   }
   produce_blocks(1);

   // iterate over scope
   mtpio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   mtpio::chain_apis::read_only::get_table_by_scope_params param{N(mtpio.token), N(accounts), "inita", "", 10};
   mtpio::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param);

   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(mtpio.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(mtpio)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, mtpio::chain::asset::from_string("10000.0000 SYS") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "mtpio")
         ("maximum_supply", mtpio::chain::asset::from_string("1000000000.0000 AAA"));
   push_action(N(mtpio.token), N(create), N(mtpio.token), act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, mtpio::chain::asset::from_string("9999.0000 AAA") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "mtpio")
         ("maximum_supply", mtpio::chain::asset::from_string("1000000000.0000 CCC"));
   push_action(N(mtpio.token), N(create), N(mtpio.token), act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, mtpio::chain::asset::from_string("7777.0000 CCC") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "mtpio")
         ("maximum_supply", mtpio::chain::asset::from_string("1000000000.0000 BBB"));
   push_action(N(mtpio.token), N(create), N(mtpio.token), act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, mtpio::chain::asset::from_string("8888.0000 BBB") );
   }
   produce_blocks(1);

   // get table: normal case
   mtpio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   mtpio::chain_apis::read_only::get_table_rows_params p;
   p.code = N(mtpio.token);
   p.scope = "inita";
   p.table = N(accounts);
   p.json = true;
   p.index_position = "primary";
   mtpio::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("mtpio", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("mtpio", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("mtpio", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("mtpio", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   set_code( config::system_account_name, contracts::mtpio_system_wasm() );
   set_abi( config::system_account_name, contracts::mtpio_system_abi().data() );
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(mtpio), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
   ...
   bidname(N(inita), N(com), mtpio::chain::asset::from_string("10.0000 SYS"));
   bidname(N(initb), N(org), mtpio::chain::asset::from_string("11.0000 SYS"));
   bidname(N(initc), N(io), mtpio::chain::asset::from_string("12.0000 SYS"));
   bidname(N(initd), N(html), mtpio::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
