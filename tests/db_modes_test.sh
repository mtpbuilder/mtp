...
MTPIO_STUFF_DIR=$(mktemp -d)
trap "rm -rf $MTPIO_STUFF_DIR" EXIT
NODMTP_LAUNCH_PARAMS="./programs/nodmtp/nodmtp -d $MTPIO_STUFF_DIR --config-dir $MTPIO_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -pmtpio"

run_nodmtp() {
   if (( $VERBOSE == 0 )); then
      $NODMTP_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $NODMTP_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $NODMTP_PID; wait $NODMTP_PID; exit 1" ALRM
   sleep 10
   if wait $NODMTP_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
