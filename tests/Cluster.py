...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="mtpio-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kmtpd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="MTPtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kmtpd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: mtp server host
        port: mtp server port
        walletHost: mtp wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures mtp mongo plugin
        ...
        self.mtpioAccount=self.defProducerAccounts["mtpio"]= Account("mtpio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodmtpArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodmtpArgs=None, onlySetProds=False,
        ...
        extraNodmtpArgs: string of arguments to pass through to each nodoes instance (via --nodmtp flag on launcher)
        ...
        specificExtraNodmtpArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodmtp flags on launcher), example: { "5" : "--plugin mtpio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the mtpio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.mtpServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodmtp default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.mtpLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodmtpArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodmtpArgs += " --plugin mtpio::wallet_api_plugin"
        if self.enableMongo:
            nodmtpArgs += " --plugin mtpio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodmtpArgs is not None:
            assert(isinstance(extraNodmtpArgs, str))
            nodmtpArgs += extraNodmtpArgs
        if Utils.Debug:
            nodmtpArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodmtpArgs += " --plugin mtpio::producer_api_plugin"

        if nodmtpArgs:
            cmdArr.append("--nodmtp")
            cmdArr.append(nodmtpArgs)

        if specificExtraNodmtpArgs is not None:
            assert(isinstance(specificExtraNodmtpArgs, dict))
            for nodeNum,arg in specificExtraNodmtpArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodmtp")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodmtp")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.mtpServerName, startedNodes, len(nodes)))
            return False
        ...
        self.mtpioAccount=self.defProducerAccounts["mtpio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.mtpClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.mtpClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on mtp server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextmtpIdx: %d, n: %d" % (nextmtpIdx, n))
                nextmtpIdx=(nextmtpIdx + 1)%count
                if not self.nodes[nextmtpIdx].killed:
                    #Utils.Print("nextmtpIdx: %d" % (nextmtpIdx))
            ...
            #Utils.Print("nextmtpIdx: %d, count: %d" % (nextmtpIdx, count))
            node=self.nodes[nextmtpIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.mtpServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on mtp node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getmtpBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.mtpioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse mtpio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by mtpio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/mtpio.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/mtpio.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down mtp cluster.")
            return None
        ...
        mtpioName="mtpio"
        mtpioKeys=producerKeys[mtpioName]
        mtpioAccount=Account(mtpioName)
        mtpioAccount.ownerPrivateKey=mtpioKeys["private"]
        mtpioAccount.ownerPublicKey=mtpioKeys["public"]
        mtpioAccount.activePrivateKey=mtpioKeys["private"]
        mtpioAccount.activePublicKey=mtpioKeys["public"]
        producerKeys.pop(mtpioName)

        if not self.walletMgr.importKey(mtpioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (mtpioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="mtpio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"mtpio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init mtpio transfer")
            opts="--permission mtpio@active"
            if name != "mtpio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from mtpio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(mtpioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, mtpioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission mtpio@active"
                    myTrans=biosNode.pushMessage("mtpio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("mtpio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but mtpio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "mtpio"
        ...
        mtpioTokenAccount=copy.deepcopy(mtpioAccount)
        mtpioTokenAccount.name="mtpio.token"
        trans=biosNode.createAccount(mtpioTokenAccount, mtpioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (mtpioTokenAccount.name))
            return None

        mtpioRamAccount=copy.deepcopy(mtpioAccount)
        mtpioRamAccount.name="mtpio.ram"
        trans=biosNode.createAccount(mtpioRamAccount, mtpioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (mtpioRamAccount.name))
            return None

        mtpioRamfeeAccount=copy.deepcopy(mtpioAccount)
        mtpioRamfeeAccount.name="mtpio.ramfee"
        trans=biosNode.createAccount(mtpioRamfeeAccount, mtpioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (mtpioRamfeeAccount.name))
            return None

        mtpioStakeAccount=copy.deepcopy(mtpioAccount)
        mtpioStakeAccount.name="mtpio.stake"
        trans=biosNode.createAccount(mtpioStakeAccount, mtpioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (mtpioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(mtpioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=mtpioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (mtpioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to mtpio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (mtpioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (mtpioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to mtpio contract.")
            return None
        ...
        Utils.Print("Verify mtpio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountmtpBalanceStr(mtpioAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (mtpioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (mtpioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (mtpioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (mtpioAccount.name)
            trans=biosNode.pushMessage(mtpioAccount.name, action, data, opts)
    ...
    @staticmethod
    def pgrepmtpServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.mtpServerName)
    ...
    @staticmethod
    def pgrepmtpServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of mtpInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepmtpServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepmtpServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.mtpServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepmtpServers(timeout=timeout)
        pattern=Cluster.pgrepmtpServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.mtpServerName, pattern))
    ...
    # Kills a percentange of mtp instances starting from the tail and update mtpInstanceInfos state
    def killSomemtpInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.mtpServerName, killSignal))
    ...
    def relaunchmtpInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodmtp instances. allInstances will kill all nodmtp instances running on the system."""
        cmd="%s -k 9" % (Utils.mtpLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down mtp cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the mtp server
            cmd="pkill -9 %s" % (Utils.mtpServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down mtp cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodmtp instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodmtp instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
