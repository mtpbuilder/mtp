...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="MTPtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getmtpAccount(account.name, exitOnError=True)
            try:
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            return self.processClmtpCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
        else:
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        if not self.enableMongo:
            ...
            for i in range(0,(int(60/timeout) - 1)):
                trans=self.processClmtpCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False):
        ...
        trans=self.processClmtpCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getmtpAccount(self, name, exitOnError=False, returnType=ReturnType.json, avoidMongo=False):
        assert(isinstance(name, str))
        if not self.enableMongo or avoidMongo:
            cmdDesc="get account"
            jsonFlag="-j" if returnType==ReturnType.json else ""
            cmd="%s %s %s" % (cmdDesc, jsonFlag, name)
            msg="( getmtpAccount(name=%s) )" % (name);
            return self.processClmtpCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg, returnType=returnType)
        else:
            assert returnType == ReturnType.json, "MongoDB only supports a returnType of ReturnType.json"
            return self.getmtpAccountFromDb(name, exitOnError=exitOnError)

    def getmtpAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processClmtpCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processClmtpCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    def verifyAccount(self, account):
        assert(account)
        if not self.enableMongo:
            ret=self.getmtpAccount(account.name)
            if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getmtpAccountFromDb(account.name)
        if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.mtpClientPath, self.mtpClientArgs(), source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getmtpBalances([source] + accounts)
    ...
    def getmtpBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountmtpBalance(account.name)
    ...
    # Get actions mapped to an account (clmtp get actions)
    ...
    def getAccountmtpBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clmtp get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("mtpio.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountmtpBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountmtpBalance(self, scope):
        """Returns SYS currency0000 account balance from clmtp get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountmtpBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.mtpClientPath, self.mtpClientArgs(), account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.mtpClientPath, self.mtpClientArgs(), account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.mtpClientPath, self.mtpClientArgs(), account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClmtpCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClmtpCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
    ...
    def processClmtpCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        assert(isinstance(returnType, ReturnType))
        cmd="%s %s %s" % (Utils.mtpClientPath, self.mtpClientArgs(), cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processClmtpCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clmtp get info."""
        if not self.enableMongo:
    ...
    # If nodmtpPath is equal to None, it will use the existing nodmtp path
    def relaunch(self, nodeId, chainArg=None, newChain=False, timeout=Utils.systemWaitTimeout, addSwapFlags=None, cachePopen=False, nodmtpPath=None):
        ...
        if nodmtpPath: splittedCmd[0] = nodmtpPath
    ...
    # Require PREACTIVATE_FEATURE to be activated and require mtpio.bios with preactivate_feature
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("mtpio", "activate", data, opts)
    ...
    def getLatestBlockHeaderState(self):
        ...
        latestBlockHeaderState = self.processClmtpCmd(cmdDesc, cmdDesc)
