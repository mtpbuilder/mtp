...
[[info]]
| Be aware that if a clear text `data` field is used, clmtp need to fetch copies of required ABIs using `nodmtp` API. That operation has a performance overhead on `nodmtp`
...
```sh
clmtp push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
clmtp push transaction JSON
```
