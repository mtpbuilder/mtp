...
### Get formatted data for user **mtpio**

```sh
clmtp get account mtpio
```
```console
privileged: true
permissions: 
...
producers:     <not voted>
```
### Get JSON data for user **mtpio**

```sh
clmtp get account mtpio --json
```
```json
{
  "account_name": "mtpio",
  ...
  "permissions": [{
      ...
      "required_auth": {
        "threshold": 1,
        "keys": [{
            "key": "MTP6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
            "weight": 1
          }
        ],
        "accounts": [],
