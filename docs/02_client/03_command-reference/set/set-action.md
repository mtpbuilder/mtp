...
```console
Usage: clmtp set action [OPTIONS] SUBCOMMAND
...
clmtp set action permission sandwichfarm mtpio.system voteproducer voting -p sandwichfarm@voting

#Now can execute the transaction with the previously set permissions. 
clmtp system voteproducer approve sandwichfarm someproducer -p sandwichfarm@voting
```
