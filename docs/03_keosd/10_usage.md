---
content_title: Kmtpd Usage
---

[[info | Recommended Usage]]
| For most users, the easiest way to use `kmtpd` is to have `clmtp` launch it automatically. Wallet files will be created in the default directory (`~/mtpio-wallet`).

## Launching kmtpd manually

`kmtpd` can be launched manually from the terminal by running:

```sh
kmtpd
```

By default, `kmtpd` creates the folder `~/mtpio-wallet` and populates it with a basic `config.ini` file.  The location of the config file can be specified on the command line using the `--config-dir` argument.  The configuration file contains the HTTP server endpoint for incoming HTTP connections and other parameters for cross-origin resource sharing.
...
By default, `kmtpd` is set to lock your wallet after 15 minutes of inactivity. This is configurable in the `config.ini` by setting the timeout seconds in `unlock-timeout`. Setting it to 0 will cause `kmtpd` to always lock your wallet.

## Stopping kmtpd

The most effective way to stop `kmtpd` is to find the kmtpd process and send a SIGTERM signal to it.

## Other options

For a list of all commands known to `kmtpd`, simply run it with no arguments:
...
Config Options for mtpio::http_plugin:
  --unix-socket-path arg (=kmtpd.sock)  The filename (relative to data-dir) to
...
Config Options for mtpio::wallet_plugin:
