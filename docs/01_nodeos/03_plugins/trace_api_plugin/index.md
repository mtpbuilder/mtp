...
```console
# config.ini
plugin = mtpio::trace_api_plugin
[options]
```
```sh
# command-line
nodmtp ... --plugin mtpio::trace_api_plugin [options]
```
...
```console
Config Options for mtpio::trace_api_plugin:
...
Here is a `nodmtp` configuration example for the `trace_api_plugin` when tracing some MTPIO reference contracts:

```sh
nodmtp --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin mtpio::trace_api_plugin 
--trace-rpc-abi=mtpio=abis/mtpio.abi 
--trace-rpc-abi=mtpio.token=abis/mtpio.token.abi 
--trace-rpc-abi=mtpio.msig=abis/mtpio.msig.abi 
--trace-rpc-abi=mtpio.wrap=abis/mtpio.wrap.abi
```
