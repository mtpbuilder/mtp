...
peer validation for similar hardware/mtpio-version/config will be <= m
...
#       * image will be rendered by gatsby-remark-graphviz plugin in mtpio docs.
...
Running wasm-runtime=mtp-vm-jit mtp-vm-oc-enable on relay node will reduce the validation time.
