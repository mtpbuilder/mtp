...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an MTPIO node.
...
```console
Config Options for mtpio::chain_plugin:
  ...
  --wasm-runtime mtp-vm|mtp-vm-jit      Override default WASM runtime (wabt)
  --mtp-vm-oc-enable                    Enable optimized compilation in WASM
