---
content_title: Nodmtp Logging
---

Logging for `nodmtp` is controlled by the `logging.json` file. CLI options can be passed to `nodmtp` to [setup `logging.json`](00_setup-logging.json.md). The logging configuration file can be used to define [appenders](#appenders) and tie them to [loggers](#loggers) and [logging levels](01_logging-levels.md).

## Appenders

The logging library built into MTPIO supports two appender types:
...
The logging library built into MTPIO currently supports five loggers:
