---
content_title: Nodmtp Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodmtp`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodmtp`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | Nodmtp is modular]]
| Plugins add incremental functionality to `nodmtp`. Unlike runtime plugins, `nodmtp` plugins are built at compile-time.
