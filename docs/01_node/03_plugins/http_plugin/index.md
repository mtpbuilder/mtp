...
The `http_plugin` is a core plugin supported by both `nodmtp` and `kmtpd`. The plugin is required to enable any RPC API functionality provided by a `nodmtp` or `kmtpd` instance.
...
```sh
# command-line
nodmtp ... --plugin mtpio::http_plugin [options]
 (or)
kmtpd ... --plugin mtpio::http_plugin [options]
```
...
Config Options for mtpio::http_plugin:
  --unix-socket-path arg                The filename (relative to data-dir) to 
                                        ...
                                        blank to disable (=kmtpd.sock for kmtpd)
  --http-server-address arg (=127.0.0.1:8888 for nodmtp)
