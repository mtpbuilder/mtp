...
namespace mtpio { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      MTP_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         MTP_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      MTP_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
