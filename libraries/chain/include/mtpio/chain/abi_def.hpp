...
extern unsigned char mtpio_abi_bin[2132];
...
datastream<ST>& operator << (datastream<ST>& s, const mtpio::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
datastream<ST>& operator >> (datastream<ST>& s, mtpio::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const mtpio::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, mtpio::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}

} // namespace fc

FC_REFLECT( mtpio::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( mtpio::chain::field_def                        , (name)(type) )
FC_REFLECT( mtpio::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( mtpio::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( mtpio::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( mtpio::chain::clause_pair                      , (id)(body) )
FC_REFLECT( mtpio::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( mtpio::chain::variant_def                      , (name)(types) )
FC_REFLECT( mtpio::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
