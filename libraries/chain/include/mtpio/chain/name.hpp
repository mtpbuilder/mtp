...
namespace fc {
  class variant;
  void to_variant(const mtpio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, mtpio::chain::name& check);
} // fc
...
namespace mtpio::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, mtpio::chain::name& check);
...
#define N(X) mtpio::chain::string_to_name(#X)

} // mtpio::chain

namespace std {
   template<> struct hash<mtpio::chain::name> : private hash<uint64_t> {
      typedef mtpio::chain::name argument_type;
...
FC_REFLECT( mtpio::chain::name, (value) )
