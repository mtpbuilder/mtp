...
#include <mtpio/chain/webassembly/wavm.hpp>
#include <mtpio/chain/webassembly/wabt.hpp>
#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
#include <mtpio/chain/webassembly/mtp-vm-oc.hpp>
#else
#define _REGISTER_MTPVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
...
using namespace fc;
using namespace mtpio::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
...
#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
      struct mtpvmoc_tier {
         mtpvmoc_tier(const boost::filesystem::path& d, const mtpvmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         mtpvmoc::code_cache_async cc;
         mtpvmoc::executor exec;
         mtpvmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool mtpvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const mtpvmoc::config& mtpvmoc_config) : db(d), wasm_runtime_time(vm) {
         if(vm == wasm_interface::vm_type::wabt)
            runtime_interface = std::make_unique<webassembly::wabt_runtime::wabt_runtime>();
#ifdef MTPIO_MTP_VM_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::mtp_vm)
            runtime_interface = std::make_unique<webassembly::mtp_vm_runtime::mtp_vm_runtime<mtpio::vm::interpreter>>();
#endif
#ifdef MTPIO_MTP_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::mtp_vm_jit)
            runtime_interface = std::make_unique<webassembly::mtp_vm_runtime::mtp_vm_runtime<mtpio::vm::jit>>();
#endif
#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::mtp_vm_oc)
            runtime_interface = std::make_unique<webassembly::mtpvmoc::mtpvmoc_runtime>(data_dir, mtpvmoc_config, d);
#endif
         if(!runtime_interface)
            MTP_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
         if(mtpvmoc_tierup) {
            MTP_ASSERT(vm != wasm_interface::vm_type::mtp_vm_oc, wasm_exception, "You can't use MTP VM OC as the base runtime when tier up is activated");
            mtpvmoc.emplace(data_dir, mtpvmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            MTP_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            MTP_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
         if(mtpvmoc) for(auto it = first_it; it != last_it; it++)
            mtpvmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  MTP_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  MTP_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
      fc::optional<mtpvmoc_tier> mtpvmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_MTP_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_MTPVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, MTPIO_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
