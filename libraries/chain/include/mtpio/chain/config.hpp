...
namespace mtpio { namespace chain { namespace config {
...
const static name system_account_name    { N(mtpio) };
const static name null_account_name      { N(mtpio.null) };
const static name producers_account_name { N(mtpio.prods) };
...
const static name mtpio_auth_scope       { N(mtpio.auth) };
const static name mtpio_all_scope        { N(mtpio.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name mtpio_any_name  { N(mtpio.any) };
const static name mtpio_code_name { N(mtpio.code) };
...
const static mtpio::chain::wasm_interface::vm_type default_wasm_runtime = mtpio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace mtpio::chain::config

constexpr uint64_t MTP_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / mtpio::chain::config::percent_100;
}

template<typename Number>
Number MTP_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + mtpio::chain::config::percent_100 - mtpio::chain::config::percent_1)  / mtpio::chain::config::percent_100;
}
