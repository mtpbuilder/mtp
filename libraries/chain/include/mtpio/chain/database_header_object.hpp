...
namespace mtpio { namespace chain {
      ...
      class database_header_object : public chainbase::object<database_header_object_type, database_header_object>
      {
         ...
         void validate() const {
            MTP_ASSERT(std::clamp(version, minimum_version, current_version) == version, bad_database_version_exception,
...
CHAINBASE_SET_INDEX_TYPE(mtpio::chain::database_header_object, mtpio::chain::database_header_multi_index)

FC_REFLECT( mtpio::chain::database_header_object, (version) )
