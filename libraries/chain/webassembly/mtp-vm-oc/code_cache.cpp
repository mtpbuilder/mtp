...
#include <mtpio/chain/webassembly/mtp-vm-oc/code_cache.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/config.hpp>
#include <mtpio/chain/webassembly/common.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/memory.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/mtp-vm-oc.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/intrinsic.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/compile_monitor.hpp>
...
static constexpr uint64_t header_id = 0x32434f4d56534f45ULL; //"MTPVMOC2" little endian
...
code_cache_async::code_cache_async(const bfs::path data_dir, const mtpvmoc::config& mtpvmoc_config, const chainbase::database& db) :
   code_cache_base(data_dir, mtpvmoc_config, db),
   _result_queue(mtpvmoc_config.threads * 2),
   _threads(mtpvmoc_config.threads)
{
   FC_ASSERT(_threads, "MTP VM OC requires at least 1 compile thread");
      ...
      if(_outstanding_compiles_and_poison[result.code] == false) {
         result.result.visit(overloaded {
            ...
            [&](const compilation_result_unknownfailure&) {
               wlog("code ${c} failed to tier-up with MTP VM OC", ("c", result.code.code_id));
   ...
   if(_outstanding_compiles_and_poison.size()) {
      ...
      while(count_processed && _queued_compiles.size()) {
         ...
         if(codeobject) {
            ...
            FC_ASSERT(write_message_with_fds(_compile_monitor_write_socket, compile_wasm_message{ *nextup }, fds_to_pass), "MTP VM failed to communicate to OOP manager");
            --count_processed;
         }
...
code_cache_sync::~code_cache_sync() {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   if(success)
      elog("unexpected response from MTP VM OC compile monitor during shutdown");
}
...
const code_descriptor* const code_cache_sync::get_descriptor_for_code_sync(const digest_type& code_id, const uint8_t& vm_version) {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   MTP_ASSERT(success, wasm_execution_error, "failed to read response from monitor process");
   MTP_ASSERT(message.contains<wasm_compilation_result_message>(), wasm_execution_error, "unexpected response from monitor process");

   wasm_compilation_result_message result = message.get<wasm_compilation_result_message>();
   MTP_ASSERT(result.result.contains<code_descriptor>(), wasm_execution_error, "failed to compile wasm");
...
code_cache_base::code_cache_base(const boost::filesystem::path data_dir, const mtpvmoc::config& mtpvmoc_config, const chainbase::database& db) :
   _db(db),
   _cache_file_path(data_dir/"code_cache.bin")
{
   ...
   if(!bfs::exists(_cache_file_path)) {
      MTP_ASSERT(mtpvmoc_config.cache_size >= allocator_t::get_min_size(total_header_size), database_exception, "configured code cache size is too small");
      std::ofstream ofs(_cache_file_path.generic_string(), std::ofstream::trunc);
      MTP_ASSERT(ofs.good(), database_exception, "unable to create MTP VM Optimized Compiler code cache");
      bfs::resize_file(_cache_file_path, mtpvmoc_config.cache_size);
      ...
      new (creation_region.get_address()) allocator_t(mtpvmoc_config.cache_size, total_header_size);
   ...
   code_cache_header cache_header;
   {
      ...
      MTP_ASSERT(!hs.fail(), bad_database_version_exception, "failed to read code cache header");
      memcpy((char*)&cache_header, header_buff + header_offset, sizeof(cache_header));
   }

   MTP_ASSERT(cache_header.id == header_id, bad_database_version_exception, "existing MTP VM OC code cache not compatible with this version");
   MTP_ASSERT(!cache_header.dirty, database_exception, "code cache is dirty");

   set_on_disk_region_dirty(true);

   auto existing_file_size = bfs::file_size(_cache_file_path);
   if(mtpvmoc_config.cache_size > existing_file_size) {
      bfs::resize_file(_cache_file_path, mtpvmoc_config.cache_size);
      ...
      resize_allocator->grow(mtpvmoc_config.cache_size - existing_file_size);
   }

   _cache_fd = ::open(_cache_file_path.generic_string().c_str(), O_RDWR | O_CLOEXEC);
   MTP_ASSERT(_cache_fd >= 0, database_exception, "failure to open code cache");

   //load up the previous cache index
   char* code_mapping = (char*)mmap(nullptr, mtpvmoc_config.cache_size, PROT_READ|PROT_WRITE, MAP_SHARED, _cache_fd, 0);
   MTP_ASSERT(code_mapping != MAP_FAILED, database_exception, "failure to mmap code cache");

   allocator_t* allocator = reinterpret_cast<allocator_t*>(code_mapping);

   if(cache_header.serialized_descriptor_index) {
      fc::datastream<const char*> ds(code_mapping + cache_header.serialized_descriptor_index, mtpvmoc_config.cache_size - cache_header.serialized_descriptor_index);
      ...
      ilog("MTP VM Optimized Compiler code cache loaded with ${c} entries; ${f} of ${t} bytes free", ("c", number_entries)("f", allocator->get_free_memory())("t", allocator->get_size()));
   }
   munmap(code_mapping, mtpvmoc_config.cache_size);

   _free_bytes_eviction_threshold = mtpvmoc_config.cache_size * .1;
