...
#include <mtpio/chain/webassembly/mtp-vm-oc/ipc_protocol.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/compile_monitor.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/ipc_helpers.hpp>
#include <mtpio/chain/webassembly/mtp-vm-oc/compile_trampoline.hpp>
...
struct compile_monitor_session {
   compile_monitor_session(boost::asio::io_context& context, local::datagram_protocol::socket&& n, wrapped_fd&& c, wrapped_fd& t) :
      _ctx(context),
      _nodmtp_instance_socket(std::move(n)),
   ...
   void read_message_from_nodmtp() {
      _nodmtp_instance_socket.async_wait(local::datagram_protocol::socket::wait_read, [this](auto ec) {
         if(ec) {
            connection_dead_signal();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodmtp_instance_socket);
         if(!success) {
            connection_dead_signal();
            return;
         }
   ...
   void kick_compile_off(const code_tuple& code_id, wrapped_fd&& wasm_code) {
      ...
      mtpvmoc_message trampoline_compile_request = compile_wasm_message{code_id};
      if(write_message_with_fds(_trampoline_socket, trampoline_compile_request, fds_pass_to_trampoline) == false) {
         ...
         write_message_with_fds(_nodmtp_instance_socket, reply);
         return;
      }
...
private:
   ...
   local::datagram_protocol::socket _nodmtp_instance_socket;
...
struct compile_monitor {
   compile_monitor(boost::asio::io_context& ctx, local::datagram_protocol::socket&& n, wrapped_fd&& t) : _nodmtp_socket(std::move(n)), _trampoline_socket(std::move(t)) {
   ...
   void wait_for_new_incomming_session(boost::asio::io_context& ctx) {
      _nodmtp_socket.async_wait(boost::asio::local::datagram_protocol::socket::wait_read, [this, &ctx](auto ec) {
         if(ec) {
            ctx.stop();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodmtp_socket);
         if(!success) {   //failure reading indicates that nodmtp has shut down
            ctx.stop();
            return;
         }
         ...
         try {
            ...
            write_message_with_fds(_nodmtp_socket, initalize_response_message());
         }
         catch(const fc::exception& e) {
            write_message_with_fds(_nodmtp_socket, initalize_response_message{e.what()});
         }
         catch(...) {
            write_message_with_fds(_nodmtp_socket, initalize_response_message{"Failed to create compile process"});
         }
   ...
   local::datagram_protocol::socket _nodmtp_socket;
   wrapped_fd _trampoline_socket;

   std::list<compile_monitor_session> _compile_sessions;
};

void launch_compile_monitor(int nodmtp_fd) {
...

   {
      ...
      boost::asio::local::datagram_protocol::socket nodmtp_socket(ctx);
      nodmtp_socket.assign(boost::asio::local::datagram_protocol(), nodmtp_fd);
      wrapped_fd trampoline_socket(socks[0]);
      compile_monitor monitor(ctx, std::move(nodmtp_socket), std::move(trampoline_socket));
      ctx.run();
      if(monitor._compile_sessions.size())
         std::cerr << "ERROR: MTP VM OC compiler monitor exiting with active sessions" << std::endl;
   }
...
wrapped_fd get_connection_to_compile_monitor(int cache_fd) {
   FC_ASSERT(the_compile_monitor_trampoline.compile_manager_pid >= 0, "MTP VM oop connection doesn't look active");
   ...
   MTP_ASSERT(success, misc_exception, "failed to read response from monitor process");
   MTP_ASSERT(message.contains<initalize_response_message>(), misc_exception, "unexpected response from monitor process");
   MTP_ASSERT(!message.get<initalize_response_message>().error_message, misc_exception, "Error message from monitor process: ${e}", ("e", *message.get<initalize_response_message>().error_message));
