...
mtpio::chain::asset core_from_string(const std::string& s) {
  return mtpio::chain::asset::from_string(s + " " CORE_SYMBOL_NAME);
}
  ...
  void base_tester::set_transaction_headers( transaction& trx, uint32_t expiration, uint32_t delay_sec ) const {
   ...
   transaction_trace_ptr base_tester::create_account( account_name a, account_name creator, bool multisig, bool include_code ) {
      ...
      if( include_code ) {
         ...
         owner_auth.accounts.push_back( permission_level_weight{ {a, config::mtpio_code_name},
                                                                 static_cast<weight_type>(owner_auth.threshold) } );
         sort_permissions(owner_auth);
         active_auth.accounts.push_back( permission_level_weight{ {a, config::mtpio_code_name},
      ...
      auto sync_dbs = [](base_tester& a, base_tester& b) {
         for( uint32_t i = 1; i <= a.control->head_block_num(); ++i ) {
            ...
            if( block ) { //&& !b.control->is_known_block(block->id()) ) {
               ...
               b.control->push_block(bsf, forked_branch_callback{}, trx_meta_cache_lookup{}); //, mtpio::chain::validation_steps::created_block);
            }
         }
      };
   ...
   void base_tester::set_before_preactivate_bios_contract() {
      set_code(config::system_account_name, contracts::before_preactivate_mtpio_bios_wasm());
      set_abi(config::system_account_name, contracts::before_preactivate_mtpio_bios_abi().data());
   }

   void base_tester::set_before_producer_authority_bios_contract() {
      set_code(config::system_account_name, contracts::before_producer_authority_mtpio_bios_wasm());
      set_abi(config::system_account_name, contracts::before_producer_authority_mtpio_bios_abi().data());
   }

   void base_tester::set_bios_contract() {
      set_code(config::system_account_name, contracts::mtpio_bios_wasm());
      set_abi(config::system_account_name, contracts::mtpio_bios_abi().data());
   }
   ...
   bool mtpio_assert_message_is::operator()( const mtpio_assert_message_exception& ex ) {
      auto message = ex.get_log().at( 0 ).get_message();
      bool match = (message == expected);
      if( !match ) {
         BOOST_TEST_MESSAGE( "LOG: expected: " << expected << ", actual: " << message );
   ...
   bool mtpio_assert_message_starts_with::operator()( const mtpio_assert_message_exception& ex ) {
      auto message = ex.get_log().at( 0 ).get_message();
      bool match = boost::algorithm::starts_with( message, expected );
      if( !match ) {
         BOOST_TEST_MESSAGE( "LOG: expected: " << expected << ", actual: " << message );
   ...
   bool mtpio_assert_code_is::operator()( const mtpio_assert_code_exception& ex ) {
      auto message = ex.get_log().at( 0 ).get_message();
      bool match = (message == expected);
      if( !match ) {
         BOOST_TEST_MESSAGE( "LOG: expected: " << expected << ", actual: " << message );
