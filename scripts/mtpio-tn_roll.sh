#!/bin/bash
#
# mtpio-tn_roll is used to have all of the instances of the MTP daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: mtpio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./mtpio-tn_roll.sh is sufficient.
#

if [ -z "$MTPIO_HOME" ]; then
    echo MTPIO_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in mtpd mtpiod nodmtp; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for mtpd or mtpiod or nodmtp
    exit 1
fi
...
if [ $DD = "all" ]; then
    for MTPIO_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $MTPIO_HOME/scripts/mtpio-tn_up.sh "$*"
    done
else
    bash $MTPIO_HOME/scripts/mtpio-tn_up.sh "$*"
fi
unset MTPIO_RESTART_DATA_DIR
