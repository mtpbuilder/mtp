...
[[ ! -d $GIT_ROOT/mtp_multiversion_builder ]] && mkdir $GIT_ROOT/mtp_multiversion_builder
# pipeline config
echo 'Reading pipeline configuration file...'
[[ -f "$RAW_PIPELINE_CONFIG" ]] && cat "$RAW_PIPELINE_CONFIG" | grep -Po '^[^"/]*("((?<=\\).|[^"])*"[^"/]*)*' | jq -c .\"mtp-multiversion-tests\" > "$PIPELINE_CONFIG"
...
# multiversion
cd $GIT_ROOT/mtp_multiversion_builder
echo 'Downloading other versions of nodmtp...'
python2.7 $GIT_ROOT/.cicd/helpers/multi_mtp_docker.py
cd $GIT_ROOT
