#include <mtpio/history_plugin/history_plugin.hpp>
#include <mtpio/history_plugin/account_control_history_object.hpp>
#include <mtpio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(mtpio::account_history_object, mtpio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(mtpio::action_history_object, mtpio::action_history_index)
...
namespace mtpio {
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing nodmtp to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               MTP_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{mtpio::chain::name(v[0]), mtpio::chain::name(v[1]), mtpio::chain::name(v[2])};
               MTP_ASSERT( fe.receiver.to_uint64_t(), fc::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               MTP_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{mtpio::chain::name(v[0]), mtpio::chain::name(v[1]), mtpio::chain::name(v[2])};
               MTP_ASSERT( fe.receiver.to_uint64_t(), fc::invalid_arg_exception,
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        MTP_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         } MTP_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            MTP_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            if (!found) {
               MTP_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
