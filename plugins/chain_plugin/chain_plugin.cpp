...
#include <mtpio/chain/fork_database.hpp>
#include <mtpio/chain/block_log.hpp>
...
#include <mtpio/chain/reversible_block_object.hpp>
#include <mtpio/chain/controller.hpp>
#include <mtpio/chain/generated_transaction_object.hpp>
#include <mtpio/chain/global_property_object.hpp>
#include <mtpio/chain/snapshot.hpp>

#include <mtpio/chain/mtpio_contract.hpp>
...
std::ostream& operator<<(std::ostream& osm, mtpio::chain::db_read_mode m) {
   if ( m == mtpio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == mtpio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == mtpio::chain::db_read_mode::READ_ONLY ) { // deprecated
      osm << "read-only";
   } else if ( m == mtpio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              mtpio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(mtpio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(mtpio::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(mtpio::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(mtpio::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, mtpio::chain::validation_mode m) {
   if ( m == mtpio::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == mtpio::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              mtpio::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(mtpio::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(mtpio::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace mtpio::chain::config;
using namespace mtpio::chain::plugin_interface;
...
:my(new chain_plugin_impl()) {
   app().register_config_type<mtpio::chain::db_read_mode>();
   app().register_config_type<mtpio::chain::validation_mode>();
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<mtpio::chain::wasm_interface::vm_type>()->value_name("runtime")->notifier([](const auto& vm){
#ifndef MTPIO_MTP_VM_OC_DEVELOPER
            //throwing an exception here (like MTP_ASSERT) is just gobbled up with a "Failed to initialize" error :(
            if(vm == wasm_interface::vm_type::mtp_vm_oc) {
               elog("MTP VM OC is a tier-up compiler and works in conjunction with the configured base WASM runtime. Enable MTP VM OC via 'mtp-vm-oc-enable' option");
               MTP_ASSERT(false, plugin_exception, "");
            }
#endif
         ...
         ("read-mode", boost::program_options::value<mtpio::chain::db_read_mode>()->default_value(mtpio::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<mtpio::chain::validation_mode>()->default_value(mtpio::chain::validation_mode::FULL),
         ...
         ("mtp-vm-oc-cache-size-mb", bpo::value<uint64_t>()->default_value(mtpvmoc::config().cache_size / (1024u*1024u)), "Maximum size (in MiB) of the MTP VM OC code cache")
         ("mtp-vm-oc-compile-threads", bpo::value<uint64_t>()->default_value(1u)->notifier([](const auto t) {
               if(t == 0) {
                  elog("mtp-vm-oc-compile-threads must be set to a non-zero value");
                  MTP_ASSERT(false, plugin_exception, "");
               }
         }), "Number of threads to use for MTP VM OC tier-up")
         ("mtp-vm-oc-enable", bpo::bool_switch(), "Enable MTP VM OC tier-up runtime")
#endif
         ;
...
if( options.count(op_name) ) { \
   ...
   for( const auto& v : ops ) { \
      container.emplace( mtpio::chain::name( v ) ); \
   } \
}
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      MTP_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         MTP_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      MTP_ASSERT( !fc::exists( file_path ), plugin_exception,
   ...
   ( builtin_protocol_feature_t codename ) -> digest_type {
      ...
      if( !res.second ) {
         MTP_ASSERT( res.first->second, protocol_feature_exception,
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if MTPIO_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "MTPIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::mtpio_root_key));
         throw;
      }
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            MTP_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         MTP_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            MTP_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               MTP_ASSERT( itr->second == item.second,
      ...
      if( options.count( "chain-threads" )) {
         ...
         MTP_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      MTP_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            MTP_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
         ...
         MTP_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         MTP_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         MTP_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         MTP_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         MTP_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         MTP_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               MTP_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               MTP_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            MTP_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            if( block_log_genesis ) {
               MTP_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  MTP_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     MTP_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               MTP_ASSERT( !block_log_chain_id, plugin_config_exception,
...
#ifdef MTPIO_MTP_VM_OC_RUNTIME_ENABLED
      if( options.count("mtp-vm-oc-cache-size-mb") )
         my->chain_config->mtpvmoc_config.cache_size = options.at( "mtp-vm-oc-cache-size-mb" ).as<uint64_t>() * 1024u * 1024u;
      if( options.count("mtp-vm-oc-compile-threads") )
         my->chain_config->mtpvmoc_config.threads = options.at("mtp-vm-oc-compile-threads").as<uint64_t>();
      if( options["mtp-vm-oc-enable"].as<bool>() )
         my->chain_config->mtpvmoc_tierup = true;
#endif
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            MTP_ASSERT( itr->second == id, checkpoint_exception,
...
{ try {
   MTP_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE || !accept_transactions(), plugin_config_exception,
...
void chain_apis::read_write::validate() const {
   MTP_ASSERT( api_accept_transactions, missing_chain_api_plugin_exception,
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      MTP_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      MTP_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         MTP_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            MTP_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quiting..."); // log string searched for in: tests/nodmtp_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/nodmtp/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}

void chain_plugin::handle_bad_alloc() {
   elog("std::bad_alloc - memory exhausted");
   //return -2 -- it's what programs/nodmtp/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   MTP_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         MTP_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,MTP
      try {
         auto symb = mtpio::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }

   try {
      return ( mtpio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      MTP_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   MTP_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      MTP_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   MTP_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   MTP_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = mtpio::chain_apis::get_abi( db, p.code );
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      MTP_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      MTP_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      MTP_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      MTP_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      MTP_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      MTP_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( mtpio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, name(scope), N(stat), [&](const key_value_object& obj){
      MTP_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_us, bool shorten_abi_errors ) {
   ...
   MTP_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   MTP_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   MTP_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const try {
   const abi_def abi = mtpio::chain_apis::get_abi(db, config::system_account_name);
   ...
   MTP_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   MTP_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  MTP_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   MTP_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
   ...
   if( block_num.valid() ) {
      ...
      } MTP_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   MTP_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   MTP_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      } MTP_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      MTP_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   MTP_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            MTP_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(mtpio.token);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   MTP_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer::create_yield_function( abi_serializer_max_time ) );
      auto action_type = abis.get_action_type(params.action);
      MTP_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
      ...
      } MTP_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      MTP_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } MTP_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on mtpio account (i.e. the system contract) and how it stores its data.
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(mtpio), N(mtpio), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
      auto it = idx.find(boost::make_tuple( t_id->id, mtpio::chain::string_to_symbol_c(4,"RAMCORE") ));
      if( it != idx.end() ) {
...
FC_REFLECT( mtpio::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
