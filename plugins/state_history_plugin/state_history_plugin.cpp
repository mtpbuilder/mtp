...
#include <mtpio/state_history_plugin/state_history_log.hpp>
#include <mtpio/state_history_plugin/state_history_serialization.hpp>
...
bool include_delta(const mtpio::chain::table_id_object& old, const mtpio::chain::table_id_object& curr) {
   return old.payer != curr.payer;
}

bool include_delta(const mtpio::chain::resource_limits::resource_limits_object& old,
                   const mtpio::chain::resource_limits::resource_limits_object& curr) {
   return                                   //
...
bool include_delta(const mtpio::chain::resource_limits::resource_limits_state_object& old,
                   const mtpio::chain::resource_limits::resource_limits_state_object& curr) {
   return                                                                                       //
...
bool include_delta(const mtpio::chain::account_metadata_object& old,
                   const mtpio::chain::account_metadata_object& curr) {
   return                                               //
       old.name != curr.name ||                         //
...
bool include_delta(const mtpio::chain::code_object& old, const mtpio::chain::code_object& curr) { //
   return false;
}

bool include_delta(const mtpio::chain::protocol_state_object& old, const mtpio::chain::protocol_state_object& curr) {
...
struct state_history_plugin_impl : std::enable_shared_from_this<state_history_plugin_impl> {
   ...
   void listen() {
      ...
      auto check_ec = [&](const char* what) {
         if (!ec)
            return;
         elog("${w}: ${m}", ("w", what)("m", ec.message()));
         MTP_ASSERT(false, plugin_exception, "unable to open listen socket");
      };
   ...
   static bool is_onblock(const transaction_trace_ptr& p) {
      ...
      if (act.account != mtpio::chain::config::system_account_name || act.name != N(onblock) ||
          act.authorization.size() != 1)
         return false;
      auto& auth = act.authorization[0];
      return auth.actor == mtpio::chain::config::system_account_name &&
             auth.permission == mtpio::chain::config::active_name;
   }
   ...
   void store_traces(const block_state_ptr& block_state) {
      ...
      for (auto& r : block_state->block->transactions) {
         ...
         MTP_ASSERT(it != cached_traces.end() && it->second.trace->receipt, plugin_exception,
      ...
      MTP_ASSERT(traces_bin.size() == (uint32_t)traces_bin.size(), plugin_exception, "traces is too big");
   ...
   void store_chain_state(const block_state_ptr& block_state) {
      ...
      auto get_table_id = [&](uint64_t tid) -> const table_id_object& {
         ...
         MTP_ASSERT(it != removed_table_id.end(), chain::plugin_exception, "can not found table id ${tid}",
                    ("tid", tid));
         return *it->second;
      };
      ...
      MTP_ASSERT(deltas_bin.size() == (uint32_t)deltas_bin.size(), plugin_exception, "deltas is too big");
      state_history_log_header header{.magic        = ship_magic(ship_current_version),
                                      .block_id     = block_state->block->id(),
...
void state_history_plugin::plugin_initialize(const variables_map& options) {
   try {
      MTP_ASSERT(options.at("disable-replay-opts").as<bool>(), plugin_exception,
