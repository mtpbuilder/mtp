...
namespace mtpio::trace_api::configuration_utils {
   ...
   chain::abi_def abi_def_from_file(const std::string& file_name, const fc::path& data_dir )
   {
      ...
      MTP_ASSERT(fc::exists(abi_path) && !fc::is_directory(abi_path), chain::plugin_config_exception, "${path} does not exist or is not a file", ("path", abi_path.generic_string()));
      try {
         abi_variant = fc::json::from_file(abi_path);
      } MTP_RETHROW_EXCEPTIONS(chain::json_parse_exception, "Fail to parse JSON from file: ${file}", ("file", abi_path.generic_string()));
   ...
   std::pair<std::string, std::string> parse_kv_pairs( const std::string& input ) {
      MTP_ASSERT(!input.empty(), chain::plugin_config_exception, "Key-Value Pair is Empty");
      auto delim = input.find("=");
      MTP_ASSERT(delim != std::string::npos, chain::plugin_config_exception, "Missing \"=\"");
      MTP_ASSERT(delim != 0, chain::plugin_config_exception, "Missing Key");
      MTP_ASSERT(delim + 1 != input.size(), chain::plugin_config_exception, "Missing Value");
