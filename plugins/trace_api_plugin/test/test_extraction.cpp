...
#include <mtpio/trace_api/test_common.hpp>
#include <mtpio/trace_api/chain_extraction.hpp>

using namespace mtpio;
using namespace mtpio::trace_api;
using namespace mtpio::trace_api::test_common;
using mtpio::chain::name;
using mtpio::chain::digest_type;
...
namespace {
   ...
   auto make_transfer_action( chain::name from, chain::name to, chain::asset quantity, std::string memo ) {
      return chain::action( std::vector<chain::permission_level> {{from, chain::config::active_name}},
                            "mtpio.token"_n, "transfer"_n, make_transfer_data( from, to, quantity, std::move(memo) ) );
   }
...
BOOST_AUTO_TEST_SUITE(block_extraction)
   ...
   BOOST_FIXTURE_TEST_CASE(basic_single_transaction_block, extraction_test_fixture)
   {
      ...
      auto actt1 = make_action_trace( 0, act1, "mtpio.token"_n );
         ...
         {
            {
               ptrx1.id(),
               {
                  {
                     0,
                     "mtpio.token"_n, "mtpio.token"_n, "transfer"_n,
                     {{ "alice"_n, "active"_n }},
                     make_transfer_data( "alice"_n, "bob"_n, "0.0001 SYS"_t, "Memo!" )
                  },
                  {
                     1,
                     "alice"_n, "mtpio.token"_n, "transfer"_n,
                     {{ "alice"_n, "active"_n }},
                     make_transfer_data( "alice"_n, "bob"_n, "0.0001 SYS"_t, "Memo!" )
                  },
                  {
                     2,
                     "bob"_n, "mtpio.token"_n, "transfer"_n,
                     {{ "alice"_n, "active"_n }},
                     make_transfer_data( "alice"_n, "bob"_n, "0.0001 SYS"_t, "Memo!" )
                  }
               }
      ...
      const block_trace_v0 expected_trace { bsp1->id, 1, bsp1->prev(), chain::block_timestamp_type(1), "bp.one"_n,
         {
            ...
            },
            {
               ptrx3.id(),
               {
                  {
                     2,
                     "fred"_n, "mtpio.token"_n, "transfer"_n,
                     {{ "fred"_n, "active"_n }},
   ...
   BOOST_FIXTURE_TEST_CASE(onerror_transaction_block, extraction_test_fixture)
   {
      ...
      auto actt1 = make_action_trace( 0, onerror_act, "mtpio.token"_n );
         ...
         {
            {
               transfer_trx.id(), // transfer_trx.id() because that is the trx id known to the user
               {
                  {
                     0,
                     "mtpio.token"_n, "mtpio"_n, "onerror"_n,
                     {{ "alice"_n, "active"_n }},
